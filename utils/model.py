...
class Loss(nn.Module):
    def __init__(self, delta=1.0, w1=1.0, w2=0.1, w3=0.5, w4=0.2):
        """
        Custom loss function for super-resolution feature mapping.

        Args:
            delta (float): Placeholder (not used in this version).
            w1 (float): Weight for L1 pixel-wise loss.
            w2 (float): Weight for perceptual loss (VGG-based).
            w3 (float): Weight for SSIM loss (Structural Similarity).
            w4 (float): Weight for Gradient Difference Loss (GDL).
        """
        super(Loss, self).__init__()
        self.w1 = w1  # L1 Reconstruction Loss
        self.w2 = w2  # Perceptual Loss (VGG)
        self.w3 = w3  # SSIM Loss
        self.w4 = w4  # Gradient Difference Loss (GDL)

        # Pretrained VGG model for perceptual loss (Feature Extraction)
    ...
    def forward(self, predictions, targets, current_step=None, total_steps=None):
        ...
        Args:
            predictions (torch.Tensor): Model output tensor (batch, 256, 256).
            targets (torch.Tensor): Ground truth tensor (batch, 256, 256).
            current_step (int, optional): Current training step.
        ...
        ssim_loss = self.compute_ssim_loss(predictions, targets)

        # Gradient Difference Loss (Smoothness)
        gdl_loss = self.compute_gdl_loss(predictions, targets)

        # Combine losses with weights
        total_loss = (
            self.w1 * rec_loss +
            self.w2 * perceptual_loss +
            self.w3 * ssim_loss +
            self.w4 * gdl_loss
        )
    ...
    def compute_perceptual_loss(self, pred, target):
        """Extract features using a VGG network and compute loss."""
        pred_vgg = self.vgg(pred.unsqueeze(1).repeat(1, 3, 1, 1))  # Convert grayscale to 3-channel
        target_vgg = self.vgg(target.unsqueeze(1).repeat(1, 3, 1, 1))
        return F.mse_loss(pred_vgg, target_vgg)

    def compute_ssim_loss(self, pred, target):
        """Compute Structural Similarity Index (SSIM) as a loss function."""
        ...
        return 1 - ssim_map.mean()  # SSIM Loss (1 - SSIM value)

    def compute_gdl_loss(self, pred, target):
        """Compute Gradient Difference Loss (GDL) to enforce smoothness."""
        ...
        gdl_x = F.l1_loss(grad_x_pred, grad_x_target)
        gdl_y = F.l1_loss(grad_y_pred, grad_y_target)
